...
| POST | `/auth/forgot-password` | mail 에 인증번호 보내서 확인 후 reset-password 로 넘어가는걸로 | No |
| POST | `/auth/verify-otp` | OTP 인증번호 확인 | No |
| POST | `/auth/reset-password` | 이메일 인증 이후 비밀번호를 리셋한다 | No |

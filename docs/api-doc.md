...
| Method | Endpoint | Description | JWT Required |
| --- | --- | --- | --- |
| GET | `/recommendations/combo` | Get algorithmically recommended meal combos based on user preferences and available menus | Yes |
| GET | `/goals/today` | Retrieve today's nutrition goal progress (calories, macros) for the status bar | Yes |
| GET | `/menus/summary` | Get today's highlighted menu items per dining hall, filtered by user preferences | Yes |
| POST | `/save-menu` | Save a recommended combo to the user's favorites | Yes |
| DELETE | `/delete-menu` | Remove a previously saved combo from the user's favorites | Yes |
| PATCH | `TBD` | TBD | TBD |

...
| GET | `/dining-halls/full` | [get-full.md](get-full.md) | Get frontend-oriented nested hall/day/menu payload | Optional JWT |
...
- `/dining-halls/full` accepts optional `Authorization: Bearer` to mark favorited items for that user. Dev-only: `ALLOW_QUERY_USER_ID=true` allows `?user_id=` when the header is omitted.
